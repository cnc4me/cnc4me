 ...
 */
function getToolOffsetRegister(group: number, toolNum: number): number {
 ...
 */
function getWorkOffsetAxisRegister(group: number, axis: string): number {
  if (group < 0 || group > 6) {
 ...
 */
function getAuxWorkOffsetAxisRegister(group: number, axis: string): number {
