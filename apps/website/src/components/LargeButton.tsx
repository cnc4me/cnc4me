...
export const LargeButton: React.FC<LargeButtonProps> = ({ onClick, label, disabled = false }) => {
  ...
  return (
    <button disabled={disabled} onClick={onClick} className={disabled ? disabledStyle : enabledStyle}>
      {label}
    </button>
  );
};
