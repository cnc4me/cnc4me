...
import { useContentSearchParam, useEditorTheme, useExampleCode, useMacroRuntime, useTabSearchParam } from "./hooks";
...
export default function App(): JSX.Element {
  ...
  const setEditorContents = (input: unknown) => editorRef.current?.setValue(String(input));
        ...
        <section className="flex flex-col w-1/2 border-r border-r-purple-600">
          <div className="flex border-b border-b-gray-900 bg-[#1E1E1E]">
            ...
            <div className="flex mr-2">
              <button onClick={() => handleResetButton()} className="px-3 my-1.5 text-white rounded-l-md bg-red-700">
                {`Reset`}
              </button>
              <button onClick={() => handleRunButton()} className="px-3 my-1.5 text-white rounded-r-md bg-green-600">
                {`Run \u2bc8`}
              </button>
            </div>
          </div>
          <MacroEditor theme={editorTheme} onMount={onEditorMount} onChange={onEditorChange} />
